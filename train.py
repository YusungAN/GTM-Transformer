...
def run(args):
    ...
    # reviews_df = pd.read_csv(Path(args.data_folder + 'gtm_product_name.csv'))
    train_loader = ZeroShotDataset(train_df[:24017], Path(args.data_folder + '/images'), gtrends, args.trend_len, reviews_df, True).get_loader(batch_size=args.batch_size, train=True)
    test_loader = ZeroShotDataset(train_df[24017:], Path(args.data_folder + '/images'), gtrends, args.trend_len, reviews_df, True).get_loader(batch_size=args.batch_size, train=True)
    ...
    checkpoint_callback = pl.callbacks.ModelCheckpoint(
        ...
        filename='gtm-summed',#model_savename+'---{epoch}---'+dt_string,
        monitor='val_mae',
        mode='min',
        save_top_k=1
    )
