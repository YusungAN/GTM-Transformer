...
def run(args):
    ...
    train_loader = ZeroShotDataset(train_df[:24000], Path(args.data_folder + '/images'), gtrends, args.trend_len, reviews_df).get_loader(batch_size=args.batch_size, train=True)
    test_loader = ZeroShotDataset(train_df[24000:], Path(args.data_folder + '/images'), gtrends, args.trend_len, reviews_df).get_loader(batch_size=args.batch_size, train=True)
